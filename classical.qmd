...
# Randomized Experiments {#sec-classical}

## Classical Randomized Experiments

In Statistics, a *random* experiment is an experiment in which the
...
Let the $W\in \mathbb{R}^N$ denote the assignment vector of a sample of
$N$ units: $W_i \in \{0,1\}$.

Whether $W$ constitutes a *randomized* experiment, depends on the
**mechanism** which assigned each unit its treatment status. In this
chapter we will examine the properties of *classical* randomized
experiments and, in particular, *completely* randomized experiments.

In the next few chapters we will assume that the receipt of treatment
and assignment into treatment are the same. In @sec-irregular we will
examine the case where they are not, considering the case of symmetric
mis-specification.
...
three restrictions are referred to as *regular assignment*.
...
*individualistic* - means that the probability of selection depends on
the (pre-treatment) characteristics of the individual, **alone**. The
second condition - *probabilistic* - requires that each unit have an (*a
priori*) non-zero probability of treatment. And the third condition -
*unconfounded* - means that treatment is independent of potential
outcomes, conditional on individual characteristics. We will define
unconfoundedness in more detail shortly.
...
### Classical annd Completely Randomized Experiment

A *classical* randomized experiment is one in which the researcher knows
and, crucially, controls the assignment mechanism in such a manner that
it constitutes the properties of regular assignment.

::: {.callout-note icon="false"}
A *classical* randomized experiment is a randomized experiment in which:

1.  assignment is regular (as above)

2.  the functional form of the assignment mechanism known and controlled
    by the researcher.
:::

A *completely* randomized experiment is a particular type of *classical*
randomized experiments in which there is a fixed, pre-determined number
of subjects due to be assigned the treatment.

::: {.callout-note icon="false"}
A *completely* randomized experiment is a *classical* randomized
experiment in which the number of units assigned to treatment is fixed
according to, $$
            1 \leq N_t \leq N-1
...
Imbens & Rubin (2015) for further reading on this topic. We will revisit
part of this material in @sec-regular.

### Unconfoundedness

It is evident from equation @ref(eq:regular1) that probability of
observing any particular allocation of treatment is independent of the
potential outcomes. This is referred to as unconfoundedness.
...
Angrist and Pischke (MM & MHE) refer to this as the **Conditional
Independence Assumption**. Implicit in their reference to
unconfoundedness as an *assumption*, is the fact that MM and MHE employ
this assumption in empirical settings that do not constitute controlled
experiments and where regular assignment assumed.

By this definition,

$$
      \mathbf{Pr}(W|X, Y(0),Y(1))=\mathbf{Pr}(W|X,Y'(0),Y'(1))
$$ for all $W$, $X$ ($N\times k$ matrix of covariates), $Y(0)$, $Y(1)$,
$Y'(0)$, and $Y'(1)$.
...
$X$-characteristics of the unit (i.e. @ref(eq:regular1)). We will relax
this assumption in @sec-statified when we consider stratified
experiments. In general, the above definition is correct, while in this
specific case conditioning on $X$ is not necessary.\
...
*observed* difference in means. This is the difference between the mean
expected value of the *observed* outcomes of the treated and control
groups.

$$
E[Y_i^{obs}|W_i=1] - E[Y_i^{obs}|W_i=0]
$$ Given the relationship between the observed and potential outcomes
described in @sec-causality, the observed difference gives us the
difference between the expected value of $Y_i(1)$ for the treated group
and $Y_i(0)$ for the control group.

$$
E[Y_i(1)|W_i=1]- E[Y_i(0)|W_i=0]
$$ We can decompose this expression by adding and subtracting the
*unobserved counterfactual* $E[Y_i(0)|W_i=1]$: the expected value of the
outcome for the treated group had they not been treated.

\$\$

\underbrace{E[Y_i(1)|W_i=1]-E[Y_i(0)|W_i=1]}*{*\tau{ATT}=E\[Y_i(1)-Y_i(0)\|W_i=1\]} +
...
describes the problem of *selection*: even without treatment there may
be a mean difference between outcome of the treated and control groups.

This particular selection term can be more accurately described as
selection *into treatment*. This is not the same as other selection
...
\underbrace{E[Y_i(1)-Y_i(0)|W_i=1]}*{*\tau{ATT}} =
\underbrace{E[Y_i(1)-Y_i(0)|W_i=0]}*{*\tau{ATU}}=\underbrace{E[Y_i(1)-Y_i(0)]}*{*\tau{ATE}}
...
both $f_{Y(1)}$ and $f_{Y(0)}$, the full marginal distributions of the
potential outcomes under regular assignment. However, it **does not**
allow us to learn about $f_{Y(1),Y(0)}$, the joint distribution of
potential outcomes.
...
<!--- Banerjee, A., Duflo, E., Glennerster, R. & Kinnan, C. 2015, "The
    ...
    22-53. --->
```
![Estimates of Quantile Treatment Effects from Banerjee *et al.* (2015,
p. 38)](Images/Banerjee_p38.png)
...
1.  They can be very expensive. The time, man-power, and direct cost of
    implementing some RCTs can run into the millions of \$s.

2.  They can be unethical. Many important research questions cannot be
...
the topic, see Deaton (2010) and ...... .

Another critique comes from James Heckman, who argues that internal
validity of RCTs is lost to their lack of **external validity**. A
...
When evaluating **finite samples**, there are two potential null
...
$H_0: Y_i(0)=Y_i(1) \quad\forall\; i=1,...,N$ - **is sharp**, we know,

| $Y^{obs}$ | $W$ | $Y(0)$                  | $Y(1)$                  |
|-----------|-----|-------------------------|-------------------------|
| 3         | 1   | [3]{style="color: red"} | 3                       |
| 6         | 0   | 6                       | [6]{style="color: red"} |
| 9         | 1   | [9]{style="color: red"} | 9                       |
| 4         | 1   | [4]{style="color: red"} | 4                       |
| 5         | 0   | 5                       | [5]{style="color: red"} |
| 2         | 0   | 2                       | [2]{style="color: red"} |
...
| $Y^{obs}$ | $W$ | $Y(0)$                  | $Y(1)$                  | [$W'$]{style="color: blue"} | [$Y^{obs'}$]{style="color: blue"} |
|-----------|-----|-------------------------|-------------------------|-----------------------------|-----------------------------------|
| 3         | 1   | [3]{style="color: red"} | 3                       | [0]{style="color: blue"}    | [3]{style="color: blue"}          |
| 6         | 0   | 6                       | [6]{style="color: red"} | [1]{style="color: blue"}    | [6]{style="color: blue"}          |
| 9         | 1   | [9]{style="color: red"} | 9                       | [1]{style="color: blue"}    | [9]{style="color: blue"}          |
| 4         | 1   | [4]{style="color: red"} | 4                       | [0]{style="color: blue"}    | [4]{style="color: blue"}          |
| 5         | 0   | 5                       | [5]{style="color: red"} | [0]{style="color: blue"}    | [5]{style="color: blue"}          |
| 2         | 0   | 2                       | [2]{style="color: red"} | [1]{style="color: blue"}    | [2]{style="color: blue"}          |

$$
T(W',Y(1),Y(0)) 
$$ Compute for **all** possible realizations of $W$, get the exact
distribution of the test static **under the sharp null hypothesis**.
...
1.  Use a normal approximation of the randomization distribution of
    $\hat{\tau}$
2.  Approximate distribution of $\hat{V}$ by chi-squared and use
    t-distribution. $$
...
From Imbens and Wooldridge (2009, p.23)[^1],

[^1]: Imbens, G.W. & Wooldridge, J.M. 2009, "Recent Developments in the
...
-   $\hat{V}^{neyman}$ is an unbiased estimator for $V$, **even with
    heterogeneous TE's**.
...
For example, (Oreopoulus, 2011, p.158)[^2]

[^2]: Oreopoulos, P. 2011, "Why Do Skilled Immigrants Struggle in the
    Labor Market? A Field Experiment with Thirteen Thousand Resumes",
    American economic journal. Economic policy, vol. 3, no. 4, pp.
    148-171.

![](Images/Oreopoulus_Balance_1.png)

These tests are typically based on the ANOVA
([AN]{style="color : blue"}alysis [O]{style="color: blue"}f
[VA]{style="color : blue"}riance) test.[^3]

[^3]: Common Alternatives include Multivariate ANOVA (MANOVA) and
    Analysis of Covariance (ANCOVA)

-   Decomposes TSS into,[^4]

[^4]: Check your EC124 notes on this.
...
For example, Dupas and Robinson (2013, p.1150)[^5]

[^5]: Dupas, P. & Robinson, J. 2013, "Why Don't the Poor Save More?
    Evidence from Health Savings Experiments", The American economic
    review, vol. 103, no. 4, pp. 1138-1171.
...
-   Balance tests provide support for unconfoundedness (i.e.
...
Since $\bar{W}=\frac{N_t}{N}$, we can show that [^6]

[^6]: Do ensure that you can show this.
...
-   $\hat{\beta}^{OLS}$ is an unbiased estimate of the (average) causal
    effect, assuming unconfoundedness (i.e. completely random
    experiment).
...
[^7]: Here I am avoiding finite sample notation. This is not a big issue
    as you can always define the expectation as an average in the finite
    sample.
...
[^8]: Deaton, A. 2010, "Instruments, Randomization, and Learning about
    Development", Journal of economic literature, vol. 48, no. 2, pp.
    424-455.

::: {.callout-tip icon="false"}
## Deaton (2010, p.442)

"However, the standard error of $\beta_1$ from the OLS regression is not
generally correct. One problem is that the variance among the
...
"If the regression ... is run with the standard heteroskedasticity
...
correction is routinely done in practice..."
:::
...
Suppose we conduct a **completely randomized experiment** in a sample
drawn at random from an infinite population. Then,
...
estimator of $\tau_{ATE}$.[^9]

[^9]: Focus on the intuition of the results, not the equation. Taken
    from Deaton's (2010, p.444) discussion of Freedman (2008). In this
    equation $X_i$ is single covariate.

-   The bias is $\psi/N$ and $\psi$ is given by, $$
                \psi = -\lim \frac{1}{N}\sum_{i=1}^N (\tau_i-\tau_{ATE})f(X_i)
    $$
-   The source of the bias lies in the correlation between the
    unit-level treatment effect and the covariates.
...
Here are the estimators:[^10]

[^10]: Subject of first seminar.
...
The question of good and bad controls is **very** important.[^11]

[^11]: Here I am being more general over the choice of the estimator
    $\hat{\tau}$ and I am approximating the distribution of the
    test-statistic with the standard normal.
...
{.allout-note icon="false"} \## Power The probability of rejecting the
null hypothesis ($H_0: \tau=0$; i.e., of no effect) when it is
**false**. $$
                \theta(\tau_0)=Pr(\text{Reject }H_0|\tau=\tau_0)
$$ :::

Recall, that we reject $H_0: \tau=0$ when [^12]

[^12]: Here I am being more general over the choice of the estimator
    $\hat{\tau}$ and I am approximating the distribution of the
    test-statistic with the standard normal.
...
Evaluation]{style="color: blue"}[^13]'

[^13]: [Banerjee, A., Duflo, E., Glennerster, R. & Kinnan, C. 2015, "The
    Miracle of Microfinance? Evidence from a Randomized Evaluation",
    American economic journal. Applied economics, vol. 7, no. 1, pp.
    22-53.]{style="color: blue"}

\par

-   Microfinance was seen as a \`silver bullet' for fighting poverty,
    but robust evidence was scarce.
-   Existing evidence based on self-selected clients - not causal.
-   [Setting:]{style="color: blue"} Hyderabad, India, capital of Andrha
    Pradesh
-   [Sample:]{style="color: blue"} randomly treat 52 (out of 104)
    neighbourhoods with new Spandana branch.
-   [Data Collection:]{style="color: blue"} survey households 18 months,
    and then 42 months after treatment (N=6,850).
-   [Research Design:]{style="color: blue"} \`\`focus here on
    reduced-form/intent-to-treat estimates'' because of spillover or
    general equilibrium effects.

*"given the sampling frame, ours will be an intent-to-treat (ITT)
...
*"The main drawback of these ITT analyses is that they do not answer
questions about causal effects of the receipt of treatment itself, only
about causal effects of the assignment to treatment." (Imbens and Rubin,
2015, p.515)*[^14]

[^14]: *More generally used in discussion of non-compliance.*
...
-   NOTE: control group is not \`untreated' as other companies enter
    both treated and control groups during this period.
...
-   Highlights after 18 months:
    ...
    1.  Decline in informal borrowing. 2. No difference in borrowed
        amount. 3. No difference in (nondurable) consumption, but
        increase in purchase of durable goods. 4. No impact on
        entrepreneurship, but invest in existing businesses. 5. Increase
        in profitability of some businesses (upper tail)

-   Highlights after 42 months:

    \par

    1.  Businesses have more assets, more businesses are more
        profitable 2. Average still small $\Rightarrow$ doesn't help
        small businesses 3. No evidence of consumption change
...
[^15]: I have use the authors' notation.

$$
            y_{ia} = \alpha + \beta \times Treat_{ia} + X_a'\gamma + \varepsilon_{ia}
$$

where $i$ represents the individual and $a$ the area (neighbourhood).
...
*"microcredit is not for every household, or even most households, and
**it does not lead to the miraculous social transformation** some
proponents have claimed...allows some households to sacrifice some
instantaneous utility (temptation goods or leisure) in order to finance
lumpy purchases, either for their home or in order to establish or
expand a business"*
...
experiment in Morocco'' by Cr'{e}pon, Devoto, Duflo & Parient'{e} (2015)
